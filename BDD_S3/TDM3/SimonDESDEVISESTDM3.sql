...
SELECT
    insee,
    SUBSTR(insee, 1, 2) AS "departement",
    nom
FROM communes;
...
SELECT
    ...
    pop_mun,
    superficie,
    CAST(pop_mun / superficie AS INTEGER) AS "densité"
FROM communes
    JOIN population USING(insee)
...
SELECT
    nom AS "nom de commune",
    nom_station AS "nom de station",
    lat as latitude,
    lon as longitude
FROM stations
...
SELECT COUNT(*) AS "Nombre total de mesure" FROM mesures_mensuelles;

-- 2)

SELECT
    AVG(valeur) AS "moyenne",
    MAX(valeur) AS "maximun",
...
SELECT
    nom_station,
    nom_polluant,
    COUNT(valeur) AS "nombre de mesure",
    AVG(valeur) AS "moyenne",
    MAX(valeur) AS "maximum",
    MIN(valeur) AS "minimum"
FROM mesures_mensuelles
...
SELECT
    ...
    polluants.code_polluant,
    COUNT(valeur)
FROM stations
    CROSS JOIN polluants
    LEFT JOIN mesures_mensuelles ON stations.code_station = mesures_mensuelles.code_station AND polluants.code_polluant = mesures_mensuelles.code_polluant
GROUP BY
    stations.code_station,
    polluants.code_polluant;